...
//$row = pg_query("SELECT p.id_produto, p.descricao  FROM  produto as  p
  //                  inner join p.produto_estoque as pe on p.id_produto = pe.id_produto
    //                inner join v.item_venda as v on v.id_estoque = pe.id_estoque
//");

$strQuery = "SELECT p.id_produto, p.descricao  FROM  produto as  p
                  inner join p.produto_estoque as pe on p.id_produto = pe.id_produto
                 inner join v.item_venda as v on v.id_estoque = pe.id_estoque";
...
$produto = $dom->createElement("produtos");
